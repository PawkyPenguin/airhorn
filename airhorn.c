...
// generate ids for graphics context and pixmap and load pixmap.
// Graphics context is for the root window.
void setup_pixmap(xcb_connection_t *connection, xcb_window_t window,
    xcb_pixmap_t *pixmap_ptr, xcb_gcontext_t *gcontext, xcb_image_t *img) {
	...
	*pixmap_ptr = xcb_generate_id(connection);
    ...
    // this segfaults, apparently img->width and img->height point to the
    // wrong place
	xcb_void_cookie_t cookie = xcb_create_pixmap(connection,
	...
	*gcontext = xcb_generate_id(connection);
    ...
    // this segfaults
	xcb_void_cookie_t put_img_cookie = xcb_image_put(connection,
	                                                 *pixmap_ptr,
	                                                 *gcontext,
	                                                 img, 0, 0, 0);
...
int main(int argc, char *argv[]) {
	...
	xcb_void_cookie_t grab_cookie = xcb_grab_button_checked(
	   connection,
	   True,
	   screen->root,
	...
	unsigned int img_dimensions = gimp_image.width
	                              * gimp_image.height
	...
	xcb_image_t *img = xcb_image_create_native(connection,
	                                           gimp_image.width,
	                                           gimp_image.height,
	...
	xcb_void_cookie_t copy_answer = xcb_copy_area_checked(connection,
	                                                      *pixmap_p,
	                                                      cursor_window,
	                                                      *gc_p,
	                                                      0, 0, 100, 100,
	...
	do {
		...
		// Poll asynchronously.
		// Unfortunately I didn't find out how to have this work with waiting
		// synchronously because the xcb manual is too good for mere humans
		// to comprehend.
